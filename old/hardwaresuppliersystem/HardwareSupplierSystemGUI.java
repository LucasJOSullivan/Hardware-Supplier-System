...
public class HardwareSupplierSystemGUI extends JFrame implements ActionListener {
    ...
    StockItem s1 = new StockItem(0, "Untitled","A","Typeless",0);
    
    private final File file = new File("Mini-Project/old.hardwaresuppliersystem/stock.data");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getActionCommand().equals("New")) {
           ...
           StockItem s = new StockItem(stockID, title, referralCode, type, value);
        ...
        else if (referralCode.length() > 1 && referralCode.isLetter(referralCode.charAt(1))) {
        ...
        for (StockItem st : foundStock)
            if (st != null && st.getStockID()==searchID)
                stockItemToAmend = st;
        ...
        Integer.parseInt(amendChoiceAsInt) = JOptionPane.showInputDialog("Select a detail to modify by entering the matching integer: \n1. Title \n2. Referral Code \n3. Type \n4 Price");
             ...
             if (type.length() == 0) {
                ...
                double newValue = Double.parseDouble(JOptionPane.showInputDialog("Enter the amended price."));

                 if (newValue.length() == 0) {
            	JOptionPane.showMessageDialog("The value must contain at least one character.", "Empty Value",JOptionPane.ERROR_MESSAGE);
                newValue = JOptionPane.showInputDialog("Enter the amended price.");
            }
            
            else if (!Double.parseDouble(newValue)) {
...
removeStockItem(ArrayList<stockItem> allStock);
    ...
    public void openFile() {
        ...
        if(!file.exists())
            ...
            try{
                ArrayList<StockItem> allStock = null;
    ...
    public void saveFile() {
        ...
        try {
            ...
            ArrayList<StockItem> allStock = new ArrayList<>();
            allStock.add(item1);
            allStock.add(item2);
